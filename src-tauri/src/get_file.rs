...
#[tauri::command]
pub fn read_dir_recursive(dir_path: String) -> Result<FileNode, String> {
    fn walk(path: &PathBuf) -> std::io::Result<Option<FileNode>> {
        ...
        if is_dir {
            ...
            // inclure uniquement les fichiers `.md`
    ...
    walk(&path)
        ...
        .and_then(|opt| opt.ok_or_else(|| "Le dossier ne contient aucun fichier .md".to_string()))
}
